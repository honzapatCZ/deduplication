...
int integer(v8::Local<v8::Value> value) {
  return value->IsUint32() && value->Uint32Value(v8::Nan::GetCurrentContext()) < INTEGER_MAX;
}
...
NAN_METHOD(deduplicate) {
  ...
  const uint32_t average = info[0]->Uint32Value(nn::Nan::GetCurrentContext());
  ...
  const uint32_t minimum = info[1]->Uint32Value(nn::Nan::GetCurrentContext());
  ...
  const uint32_t maximum = info[2]->Uint32Value(nn::Nan::GetCurrentContext());
  ...
  uint32_t sourceOffset = info[4]->Uint32Value(nn::Nan::GetCurrentContext());
  if (!integer(info[5])) {
    return Nan::ThrowError("sourceSize must be an unsigned 31 bit integer");
  }
  const uint32_t sourceSize = info[5]->Uint32Value(nn::Nan::GetCurrentContext());
  ...
  uint32_t targetOffset = info[7]->Uint32Value(nn::Nan::GetCurrentContext());
  ...
  const uint32_t flags = info[8]->Uint32Value(nn::Nan::GetCurrentContext());
  if (flags != 0 && flags != 1) {
...
NAN_METHOD(targetSize) {
  ...
  const uint32_t minimum = info[0]->Uint32Value(nn::Nan::GetCurrentContext());
  if (minimum < MINIMUM_MIN) return Nan::ThrowError("minimum < MINIMUM_MIN");
  if (minimum > MINIMUM_MAX) return Nan::ThrowError("minimum > MINIMUM_MAX");
  if (!integer(info[1])) {
    return Nan::ThrowError("sourceSize must be an unsigned 31 bit integer");
  }
  const uint32_t sourceSize = info[1]->Uint32Value(nn::Nan::GetCurrentContext());
